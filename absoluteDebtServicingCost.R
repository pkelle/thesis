...
plot = layout(plot, font=list(size=14), yaxis = list(title="Gross Government Debt (Bil. Euro)", range=c(0,400)),  
              xaxis = list(autotick=F, dtick=2, title="Year"), yaxis2 = list(overlaying = "y", side = "right", title = "Interst Payments (Bil. Euro)", autotick=T, dtick=2, rangemode = "tozero", range=c(0,20)),
