...
#forecasted Greek 10yr bond yields, interest rate for new debt
...
for(i in seq(nrow(ForecastGDPmatrix))) {
  ...
  for (j in seq(ncol(ForecastGDPmatrix))) {
    ...
    #fixed interest rate
    interestpayments = avginterestrate * (temp_debt[j]-sum(temp_yearly_newdebt)) + sum((GreekNewDebtInterestRates[1:j]*temp_yearly_newdebt[1:j]))
